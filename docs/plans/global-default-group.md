...
| 3 | Alta | Condición de carrera | `UserRepositoryImpl.kt:421-427` — `role` se persiste antes de que `ensureGlobalGroupMembership` termine; fallo se traga en silencio (`runCatching`/`Timber.w`), nunca se reintenta | Antigravity (verificado por Claude) | Pendiente |
| 4 | Media | Condición de carrera | `functions/src/globalGroup.ts:24-27` — `seedGlobalStanding` hace `get()`+`set()` no transaccional; puede pisar puntos si el scoring corre en medio | Antigravity (verificado por Claude) | Pendiente |
| 5 | Crítica (lógica de negocio) | Pérdida de datos | `functions/src/inactivity.ts:48-54` — `.delete()` del standing completo; ninguna reactivación lo restaura, `weeklyBreakdown`/`totalPoints` históricos se pierden para siempre | Codex + Antigravity + Claude (verificado) | Pendiente |
| 6 | Media (diseño) | Escalabilidad | Un solo documento (`groups/global_nflocos_de_corazon`) acumula el UID de *todos* los usuarios de la app en `memberIds[]` — límite de Firestore de ~1 MiB por doc (~25-30k UIDs) y contención recomendada de ~1 escritura/seg por documento | Antigravity | Pendiente, no bloqueante hoy |
...
### Hallazgos #3-#6 — pendientes, requieren decisión de diseño

A diferencia de #1 y #2 (candados de reglas), estos requieren decidir comportamiento antes de programarlos: #3 y #4 necesitan retry/transacción; #5 necesita decidir si el standing se archiva en vez de borrarse (o si la pérdida es aceptable); #6 necesita decidir el modelo de datos a largo plazo si la base de usuarios crece más allá de ~25k. Se documentan aquí para no perderlos, pero no forman parte de esta corrección inmediata.
